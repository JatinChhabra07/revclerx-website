'use client'
/** HeroParallax — 3 rows of images scrolling in opposite directions (Aceternity hallmark). */
import { motion, useScroll, useSpring, useTransform } from 'framer-motion'
import { useRef } from 'react'
import Image from 'next/image'
import { cn } from '@/lib/utils'

interface Product { title: string; link?: string; thumbnail: string }

export function HeroParallax({ products, className }: { products: Product[]; className?: string }) {
    ...
    <div ref={ref} className={cn('h-[300vh] py-40 overflow-hidden antialiased relative flex flex-col self-auto [perspective:1000px] [transform-style:preserve-3d]', className)}>
      <motion.div style={{ rotateX, rotateZ, translateY, opacity }} className="">
        <motion.div className="flex flex-row-reverse space-x-reverse space-x-20 mb-20">
          {firstRow.map((p, i) => <ProductCard key={i} product={p} translate={translateX} />)}
        </motion.div>
        <motion.div className="flex flex-row mb-20 space-x-20">
          {secondRow.map((p, i) => <ProductCard key={i} product={p} translate={translateXReverse} />)}
        </motion.div>
        <motion.div className="flex flex-row-reverse space-x-reverse space-x-20">
          {thirdRow.map((p, i) => <ProductCard key={i} product={p} translate={translateX} />)}
        </motion.div>
      </motion.div>
    </div>
  )
...
function ProductCard({ product, translate }: { product: Product; translate: ReturnType<typeof useSpring> }) {
  return (
